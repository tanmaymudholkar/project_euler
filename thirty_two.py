...
import factorization, utilities

def is_pandigital_product(candidate):
    candidate_list_of_digits = utilities.get_list_of_digits(candidate)
    
    if (len(candidate_list_of_digits) != len(set(candidate_list_of_digits)) or 0 in candidate_list_of_digits):
        return False

    candidate_list_of_factors = factorization.get_all_factors(candidate)
    for factor in candidate_list_of_factors:
        if (factor <= candidate//2):
            complete_list_of_digits = candidate_list_of_digits + utilities.get_list_of_digits(factor) + utilities.get_list_of_digits(candidate//factor)
